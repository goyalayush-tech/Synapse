...
mod tests {
    ...
    fn test_allowlist_get_by_name() {
        ...
        allowlist.add_entry(AllowlistEntry::new(
            "agent-1",
            BinaryHash::from_bytes(b"agent1"),
        ));
        allowlist.add_entry(AllowlistEntry::new(
            "agent-2",
            BinaryHash::from_bytes(b"agent2"),
        ));
    ...
    fn test_allowlist_cgroup_check() {
        ...
        let entry =
            AllowlistEntry::new("test", BinaryHash::from_bytes(b"test")).with_cgroup("/docker");
        let hash = entry.hash.clone();
