...
impl AttestationVerifier {
    ...
    async fn get_process_info(&self, pid: u32) -> Result<ProcessInfo> {
        ...
        // Read exe path
        let exe_path = fs::read_link(proc_dir.join("exe")).await
        ...
        // Compute binary hash
        let binary_hash = BinaryHash::from_file(&exe_path).await
            .map_err(|e| VerifierError::ReadError {
                pid,
        ...
        let comm = fs::read_to_string(proc_dir.join("comm")).await
        ...
        // Read cgroup
        let cgroup = fs::read_to_string(proc_dir.join("cgroup")).await
            .map(|s| {
        ...
        let status = fs::read_to_string(proc_dir.join("status")).await
            .unwrap_or_default();
        ...
        let stat = fs::read_to_string(proc_dir.join("stat")).await
            .unwrap_or_default();
        let start_time = stat.split_whitespace()
...
mod tests {
    ...
    async fn test_verify_binary_not_allowed() {
        ...
        tokio::fs::write(&temp_file, b"test binary content").await.unwrap();
