...
impl PolicyEngine {
    ...
    /// Register a policy
    pub async fn register<P: Policy + 'static>(
        &self,
        policy: P,
    ) -> PolicyEngineResult<()> {
        ...
        policies.insert(id.clone(), PolicyEntry {
    ...
    pub async fn list(&self) -> Vec<PolicyMetadata> {
        let policies = self.policies.read().await;
        policies.values().map(|e| e.policy.metadata().clone()).collect()
    }
    ...
    pub async fn evaluate(
        ...
        id: &str,
        ctx: &PolicyContext,
    ) -> PolicyEngineResult<Verdict> {
        let start = Instant::now();
        
        let (policy, policy_timeout) = {
            let policies = self.policies.read().await;
            let entry = policies.get(id)
    ...
    pub async fn evaluate_all(
        ...
        policies.sort_by(|a, b| {
            b.1.metadata().priority.cmp(&a.1.metadata().priority)
        });
    ...
    pub async fn evaluate_until_denied(
        &self,
        ctx: &PolicyContext,
    ) -> PolicyEngineResult<Verdict> {
        let policies: Vec<_> = {
...
mod tests {
    use super::*;
    use crate::policy::{AllowAllPolicy, DenyAllPolicy, ActionFilterPolicy};
    ...
    async fn test_register_and_get() {
        ...
        engine.register(AllowAllPolicy::new()).await.expect("should register");
    ...
    async fn test_evaluate_single() {
        let engine = PolicyEngine::new(PolicyConfig::default());
        engine.register(AllowAllPolicy::new()).await.expect("register");
        engine.register(DenyAllPolicy::new("test")).await.expect("register");
    ...
    async fn test_evaluate_all() {
        let engine = PolicyEngine::new(PolicyConfig::default());
        engine.register(AllowAllPolicy::new()).await.expect("register");
        engine.register(ActionFilterPolicy::new(vec!["read".to_string()])).await.expect("register");
