...
impl HyperState {
    ...
    pub fn new(vector_dimension: usize) -> Self {
        Self {
            vector: Arc::new(RwLock::new(
                syn_memory::vector::InMemoryVectorMemory::new(vector_dimension)
            )),
            crdt: Arc::new(RwLock::new(
                syn_memory::CrdtBlackboard::new(syn_memory::CrdtConfig::default())
            )),
    ...
    pub async fn init_embedder(&self) -> anyhow::Result<()> {
        ...
        let embedder = syn_memory::Embedder::new(config).await
    ...
    pub async fn ingest(&self, event_type: &str, content: &str) -> Result<()> {
        ...
        let embedding_vec = {
            ...
            if let Some(ref embedder) = *embedder_guard {
                embedder.embed(content).await.unwrap_or_else(|_| vec![0.0f32; self.dimension])
            } else {
        ...
        let id = format!("{}_{}", event_type, chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
        ...
        self.vector.write().await.store(embedding).await
        ...
        if event_type.starts_with("state.") {
            ...
            blackboard.set(&doc_id, &[key], syn_memory::CrdtValue::String(content.to_string())).await?;
        }
    ...
    pub async fn recall(&self, query: &str, limit: usize) -> Result<Vec<String>> {
        ...
        let query_vec = {
            let embedder_guard = self.embedder.read().await;
            if let Some(ref embedder) = *embedder_guard {
                embedder.embed(query).await.unwrap_or_else(|_| vec![0.0f32; self.dimension])
            } else {
                vec![0.0f32; self.dimension]
            }
        };
        ...
        let results = memory.search(&query_vec, limit, 0.0).await
            .map_err(|e| anyhow::anyhow!("Search error: {}", e))?;
        
        Ok(results.into_iter()
            .filter_map(|r| r.embedding.metadata.get("content")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()))
            .collect())
    }
    ...
    pub async fn merge_remote(&self, message: syn_memory::SyncMessage) -> Result<Option<syn_memory::SyncMessage>> {
        let blackboard = self.crdt.read().await;
        blackboard.receive_sync_message(message).await
    ...
    pub fn new(config: NodeConfig) -> Result<Self> {
        ...
        let identity: Arc<dyn syn_identity::provider::IdentityProvider> = {
            ...
            if config.enable_ebpf {
                // Real eBPF identity on Linux - use mock for now until eBPF loader is complete
        ...
        #[cfg(feature = "policy")]
        let policy_engine = {
            let engine_config = syn_policy::engine::EngineConfig::default();
